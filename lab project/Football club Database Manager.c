...
#include<conio.h>//for clrscr,getch, color function
...
//player details in the structure
struct player
{
    char name[20];
    float height;
...
int main()
{
    ...
    gotoxy(50,10);
    red();
    //for the rolling text (print character by character)
...
void register_()
{
    ...
    reset();
    getch();
    login();//after saving the user name and pass, take user to the login interface
...
void  login()
{
    ...
    printf("Username : ");
    scanf("%s",u_id);
    while(fscanf(fp,"%s %s",&usern,&pass)!=EOF)// searching until end of the file
    {
        if(strcmp(u_id,usern)==0)
        {
            ...
            else
            {
                ...
                reset();
                getch();
                login();//if the pass don't match then send user to the login interface again
            }
        }
        gotoxy(50,10);
        reset();
        ...
        reset();
        getch();
        login();//taking user to the login function if user name don't match
    }
}
//lock function for login interface and select from login and registration
void lock()
{
    char name[30];
    int id;
    ...
    case 1:
        ...
        gotoxy(50,3);
        printf("PLAYER LOGIN");
        login();//calling login function (if user press 1)
        break;

    case 2:
        system("cls");
        register_();//calling registration(if user press 2)
        break;
...
void menu()
{
    ...
    printf("Enter your choice: ");
    scanf("%d",&choice);
    switch(choice)//take user to the function they select from the option
    {
    case 1:
        add();
        break;
...
void add()
{
    ...
    while(another == 'y')
    {
        ...
        gotoxy(10,20);
        printf("Want to add of another record? Then press 'y' else 'n'.");//after adding details giving user to add more or go to menu()
        fflush(stdin);
...
void view()
{
    ...
    gotoxy(10,7);
    printf("--------------------------------------------------------------------------------------------------------------------");
    fp = fopen("record.txt","rb+");//opening file to read the details previously added by user
    if(fp == NULL)
    {
        gotoxy(10,9);
...
//modify any player by name (search the player and then add details)
void modify()
{
    char plname[20];
    FILE *fp;
...
void deleterec()
{
    ...
    fflush(stdin);
    gets(plname);
    fp = fopen("record.txt","rb+");//opening the main file with player details
    if(fp == NULL)
    {
        gotoxy(10,6);
        printf("Error opening file");
    ...
    ft = fopen("temp.txt","wb+");//opening a temporary file
    if(ft == NULL)
    {
        gotoxy(10,6);
        printf("Error opening file");
